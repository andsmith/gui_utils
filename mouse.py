...
class MouseState(object):
    MOUSE_BUTTON_EVENTS = {cv2.EVENT_LBUTTONDOWN: {'ind': 0, 'state': 1, 'desc': "l-down"},
    ...
    def update_state(self, event, x, y, flags, param):
        ...
        if event in self.MOUSE_BUTTON_EVENTS:

            self._buttons[self.MOUSE_BUTTON_EVENTS[event]['ind']] = \
                self.MOUSE_BUTTON_EVENTS[event]['state']
            button_change = self.MOUSE_BUTTON_EVENTS[event]['desc']
            print(button_change)
